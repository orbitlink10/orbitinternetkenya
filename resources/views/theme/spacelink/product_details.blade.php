...
@php
    $productExcerpt = \Illuminate\Support\Str::squish(strip_tags($product->meta_description ?: ($product->short_description ?? $product->name)));
    $productExcerpt = \Illuminate\Support\Str::limit($productExcerpt, 155, '');
@endphp
@section('title', $product->meta_title ?: $product->name)
@section('meta_description', $productExcerpt)

@section('og_title', $product->meta_title ?: $product->name)
@section('og_description', $productExcerpt)
...
@section('twitter_description', $productExcerpt)
                            ...
                            <div id="modalCarousel" class="carousel slide" data-bs-ride="carousel">
                                <div class="carousel-inner" id="modalCarouselInner">
                                    @if($mediafiles->count() > 0)
                                        @foreach($mediafiles as $index => $media)
                                            <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                                <img src="{{ $media->file_path }}" alt="{{ $product->name }} gallery image" class="d-block w-100" style="height: 500px; object-fit: cover;">
                                            </div>
                                        @endforeach
                                    @else
                                        <div class="carousel-item active">
                                            <img src="{{ url('/') }}/storage/{{ $product->photo }}" alt="{{ $product->name }}" class="d-block w-100" style="height: 500px; object-fit: cover;">
                                        </div>
                                    @endif
                                </div>
                                <!-- Carousel Controls -->
            ...
            <div class="col-lg-6">
                <div class="detail-info">
                    ...
                    <div class="short-desc mt-3 mb-4">
                        @php
                            $shortDesc = Str::words(strip_tags($product->description), 40, '...');
                        @endphp
                    ...
                    <div class="container">
                        <div id="homepage-description">
                            @php
                                ...
                                if ($prodDesc) {
                                    $prodDesc = preg_replace('/<h1([^>]*)>/i', '<h2$1>', $prodDesc);
                                    $prodDesc = preg_replace('/<\\/h1>/i', '</h2>', $prodDesc);
                                }
                            @endphp
                            {!! $prodDesc !!}
                        </div>
