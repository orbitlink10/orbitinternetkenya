...
@php
    $catMeta = \Illuminate\Support\Str::squish(strip_tags($category->meta_description ?: $category->name));
    $catMeta = \Illuminate\Support\Str::limit($catMeta, 155, '');
@endphp
@section('title')
...
<section class="py-5" id="homepage-description">
    <div class="container">
      @php
          ...
          if ($catDescription) {
              $catDescription = preg_replace('/<h1([^>]*)>/i', '<h2$1>', $catDescription);
              $catDescription = preg_replace('/<\\/h1>/i', '</h2>', $catDescription);
          }
      @endphp
      {!! $catDescription !!}
    </div>
